...
function LoginScreen({ navigation }) {
  ...
  const passwordCheck = (val) => {
    if (val.indexOf(" ") >= 0) {
      setErrors({
        ...errors,
        errorPassword: true,
        pwErrorMsg: "Password can't contain white space",
      });
  ...
  const checkResetPassword = (pw) => {
    if (pw.indexOf(" ") >= 0) {
      setErrors({
        ...errors,
        errorResetPw: true,
        resetPwMsg: "Password can't contain white space",
      });
    } else if (pw == "" || pw == undefined || pw.length < 8 || pw.length > 16) {
      setErrors({
        ...errors,
        errorResetPw: true,
  ...
  const checkResetConfirmPassword = (confirmPw) => {
    if (confirmPw.indexOf(" ") >= 0) {
      setErrors({
        ...errors,
        errorResetConfPw: true,
        resetConfPwMsg: "Password can't contain white space",
      });
    } else if (
      confirmPw == "" ||
      confirmPw == undefined ||
  ...
  const handleNext = () => {
    ...
    if (errors.errorResetEmail) {
      ...
      setProgress({
        progress: true,
        progressMessage: "Sending reset code! Wait for second...",
      });
  ...
  const checkRegPassword = (val) => {
    if (val.indexOf(" ") >= 0) {
      setErrors({
        ...errors,
        errorRegPw: true,
        msgRegPw: "Password can't have any white spaces",
      });
    } else if (
      val.length < 8 ||
      val.length > 18 ||
  ...
  const checkRegConfPassword = (val) => {
    if (val.indexOf(" ") >= 0) {
      setErrors({
        ...errors,
        errorRegConfPw: true,
        msgRegConfPw: "Confirm Password can't have any white spaces",
      });
    } else if (
      val.length < 8 ||
      val.length > 18 ||
        ...
        >
          <View style={styles.passwordModal}>
            ...
            <View style={styles.forgotContainer}>
              ...
              >
                Enter Your Email and we will send you code to reset your
                password.
              </Text>
              <View
                style={{
