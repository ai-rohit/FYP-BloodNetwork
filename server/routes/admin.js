...
router.get("/update_hospital/:id",(req, res)=>{
    ...
    db.query("Select * from blood_hospitals where bankId=?",[id],(error, results)=>{
        console.log(results);
        if(error){
...
router.get("/update_campaign/:id",(req, res)=>{
    ...
    db.query("Select * from campaign_details where campaignId=?",[id],(error, results)=>{
        console.log(results);
        if(error){
            return res.send({status:"error", message:error.message});
        }
...
router.post("/add_campaigns",isLoggedIn, campaignsAuthorization, validationRules(), (req, res)=>{
...
router.post("/add_hospitals",isLoggedIn, (req, res)=>{
    ...
    // }
    try{
        db.query("Insert into blood_hospitals set ?", [hospitalDetails], (error, hospital)=>{
            if(error){
                return res.status(400).send({status: "error", message: error.message});
            }
